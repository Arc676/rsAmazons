...
pub mod Amazons {
    ...
    impl BoardState {
        pub fn init(
            ...
            board_width: u32,
            board_height: u32,
            white_starting: &Vec<(u32, u32)>,
            black_starting: &Vec<(u32, u32)>,
        ) {
            let mut white: Vec<Square> = white_starting
                .iter()
                .map(|coords| Square::new(coords.0, coords.1))
                .collect();
            let mut black: Vec<Square> = black_starting
                .iter()
                .map(|coords| Square::new(coords.0, coords.1))
                .collect();
            unsafe {
                boardstate_init(
                    self,
                    white_pieces as c_int,
                    black_pieces as c_int,
                    board_width as c_int,
                    board_height as c_int,
                    white.as_mut_ptr(),
                    black.as_mut_ptr(),
                );
            }
